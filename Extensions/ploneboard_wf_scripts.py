...
def notifyConvMembersByMail(self, state_change, **kw):
    ...
    comments = portal.portal_catalog.searchResults(portal_type="PloneboardComment", path="/".join(container.getPhysicalPath()))
    i = 0
...
def notifyConvMembersByMailRIC(self, state_change, **kw):
    """
        The same function as the preceding except acquisition is removed on each comment
    """
    object = state_change['object']
    portal = getToolByName(self, 'portal_url').getPortalObject()
